...
class StoryGenerationViewModel: ObservableObject {
    ...
    func generateStory(profile: UserProfile, options: StoryOptions?) async {
        ...
        let story = createPersonalizedStory(for: profile, with: storyOptions)
    ...
    private func createPersonalizedStory(for profile: UserProfile, with options: StoryOptions) -> Story {
        let storyBuilder = StoryBuilder(profile: profile, options: options)
        return storyBuilder.generateStory()
    }
}
